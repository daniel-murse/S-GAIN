...
def tf_mask_and_weight_regrow_randomly_normal_xavier_init_count(weights, mask, regrow_count, seed=None):
    ...
    inactive_indices = tf.where(tf.equal(mask_flat, 0))
    ...
    regrow_weights = tf.random.uniform([regrow_count], -limit, limit, dtype=weights.dtype, seed=seed)

    # Update weights and mask by scattering updates to the old weights and mask with the regrown indices and values
    new_weights_flat = tf.tensor_scatter_nd_update(weights_flat, regrow_indices, regrow_weights)
    new_mask_flat = tf.tensor_scatter_nd_update(mask_flat, regrow_indices, tf.ones([regrow_count], dtype=mask.dtype))

    new_weights = tf.reshape(new_weights_flat, tf.shape(weights))
    new_mask = tf.reshape(new_mask_flat, tf.shape(mask))

    return new_weights, new_mask

class RandomNormalXavierRegrowStrategy(RegrowStrategy):
    def __init__(self, regrow_count_func, period_func):
        super().__init__()
    ...
    def get_tf_regrowed_mask_and_weight_tensors(self, training_loop_iteration, weight_tensors, mask_tensors):
        if(self.period_func(training_loop_iteration)):
            return [tf_mask_and_weight_regrow_randomly_normal_xavier_init_count(w, m, rc) for (w, m), rc in zip(zip(weight_tensors, mask_tensors), self.regrow_count_func) if rc is not None]
        return None