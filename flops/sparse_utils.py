...
def get_sparsities_erdos_renyi(all_masks, default_sparsity, custom_sparsity_map, include_kernel,
                               extract_name_fn=mask_extract_name_fn, erk_power_scale=DEFAULT_ERK_SCALE):
    ...
    dense_layers = set()
    while not is_eps_valid and custom_sparsity_map is None:
        ...
        for mask in all_masks:
            ...
            if var_name in dense_layers:
                ...
                if include_kernel:
                    raw_probabilities[mask.name] = (np.sum(shape_list) / np.prod(shape_list)) ** erk_power_scale
                else:
        ...
        if max_prob_one > 1:
            ...
            for mask_name, mask_raw_prob in raw_probabilities.items():
                if mask_raw_prob == max_prob:
                    ...
                    print(f'Sparsity of var: {var_name} had to be set to 0.')
    ...
    for mask in all_masks:
        ...
        if var_name in custom_sparsity_map:
            ...
            print(f'layer: {var_name} has custom sparsity: {sparsities[mask.name]}')
        ...
        print(f'layer: {var_name}, shape: {mask.shape}, sparsity: {sparsities[mask.name]}')
    return sparsities


def get_sparsities_uniform(all_masks, default_sparsity, custom_sparsity_map, extract_name_fn=mask_extract_name_fn):
    """Given the method, returns the sparsity of individual layers as a dict.
...
def get_sparsities(all_masks, method, default_sparsity, custom_sparsity_map, extract_name_fn=mask_extract_name_fn,
    ...
    if keys_found != keys_given:
        ...
        raise ValueError(f'No masks are found for the following names: {str(diff)}')
    ...
    if method in ('erdos_renyi', 'erdos_renyi_kernel'):
        ...
        raise ValueError(f'Method: {method} is not valid mask initialization method')
    return sparsities


def get_stats(masked_layers, default_sparsity=0.8, method='erdos_renyi', custom_sparsities=None, width=1.,
              first_layer_name='conv1', last_layer_name='conv_preds', param_size=32, erk_power_scale=DEFAULT_ERK_SCALE):
    ...
    """
    if custom_sparsities is None: custom_sparsities = {}
    sparsities = get_sparsities([l.kernel for l in masked_layers], method, default_sparsity, custom_sparsities,
                                lambda a: a, erk_power_scale=erk_power_scale)
    total_flops = 0
    total_param_bits = 0
    total_params = 0.
    n_zeros = 0.
    for layer in masked_layers:
        ...
        if isinstance(layer, tf.keras.layers.Dense):
            layer_op = counting.FullyConnected(k_shape, True, 'relu' if kernel.name != 'G_W3' else 'sigmoid')
        else:
            raise ValueError('Should not happen.')
        param_count, n_mults, n_adds = counting.count_ops(layer_op, sparsities[kernel.name], param_size)
